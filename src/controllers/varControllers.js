...
export async function addVariableViaToken(req, res) {
  ...
  try {
    ...
    return res.json({ message: "Variable is added", variable: variable_name, 
                      value: value,
                      type: variable_type,
                      unit: unit
                    });
...
export async function updateVariable(req, res) {
  // Get the user's input
  // Extract the Basic Auth Credentials from the Authorization header
  const authHeader = req.headers['authorization'];
  if (!authHeader) {
...
export async function readVariableAll(req, res) {
  // Get the user's input
   // Extract the Basic Auth Credentials from the Authorization header
   const authHeader = req.headers['authorization'];
   if (!authHeader) {
  ...
  if (req.params.format == "format1") {
    const AllVariables = await MODEL.getAllVariables(stored_user_id)
...
export async function readVariableAllViaToken(req, res) {
  // Get the user's input
  // Extract the Bearer token from the Authorization header
  const authHeader = req.headers['authorization'];
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
